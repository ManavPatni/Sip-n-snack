...
class Bill_App:
    def __init__(self,root):
        ...
        self.customer=StringVar()      #customer
        self.class_c=StringVar()      #class
        ...
        o_info.place(x=10,y=5,width=600,height=140)
        ...
        #customer name
        self.customer_name = Label(o_info,text="Customer Name", font=("arial",12,"bold"),bg="white")
        self.customer_name.grid(row=0,column=2,stick=W,padx=5,pady=2)
        #customer name entry
        self.customer_ent = ttk.Entry(o_info, font=("arial",10),textvariable=self.customer,width=24)
        self.customer_ent.grid(row=0,column=3)

        #customer class
        self.customer_name = Label(o_info,text="Class", font=("arial",12,"bold"),bg="white")
        self.customer_name.grid(row=1,column=2,stick=W,padx=5,pady=2)
        #customer class entry
        self.customer_ent = ttk.Entry(o_info, font=("arial",10),textvariable=self.class_c,width=24)
        self.customer_ent.grid(row=1,column=3)
        #======================================#
        ...
        self.s_cat.grid(row=0,column=0,stick=W,padx=5,pady=2)
        #snacks category combobox
        self.san_com = ttk.Combobox(i_info, font=("arial",10),value=self.s_snack,textvariable=self.product,width=24)
        self.san_com.grid(row=0,column=1)
        self.san_com.bind("<<ComboboxSelected>>",self.price)

        #drinks category
        self.d_cat = Label(i_info,text="Drinks", font=("arial",12,"bold"),bg="white")
        self.d_cat.grid(row=1,column=0,stick=W,padx=5,pady=2)
        #drinks category combobox
        self.dri_com = ttk.Combobox(i_info, font=("arial",10),value=self.s_drink,textvariable=self.product2,width=24)
        self.dri_com.grid(row=1,column=1)
        ...
        #============Add/remove item frame===========#
        aif = LabelFrame(main_frame,text="Add / Remove Item", font=("times new roman",12,"bold"),bg="white", fg="red")
        aif.place(x=10,y=420,width=600,height=100)

        #notice
        notice = Label(main_frame,text="Currently Unavailable",fg="RED",bg="white",font=(20))
        notice.place(x=20, y=450,width=585,height=50)

        #add item
        self.add_item = Label(aif,text="Add Item", font=("arial",12,"bold"),bg="white")
        self.add_item.grid(row=0,column=0,sticky=W,padx=5,pady=2)
        #add item entry
        self.add_item_e = ttk.Entry(aif, font=("arial",10),width=24)
        self.add_item_e.grid(row=0,column=1)

        #add price
        self.add_price = Label(aif,text="Add Price", font=("arial",12,"bold"),bg="white")
        self.add_price.grid(row=0,column=2,sticky=W,padx=5,pady=2)
        #add price entry
        self.add_price_e = ttk.Entry(aif, font=("arial",10),width=24)
        self.add_price_e.grid(row=0,column=3)

        #remove item
        self.remove_item = Label(aif,text="Remove Item", font=("arial",12,"bold"),bg="white")
        self.remove_item.grid(row=1,column=0,sticky=W,padx=5,pady=2)
        #remove item entry
        self.remove_item_e = ttk.Entry(aif, font=("arial",10),width=24)
        self.remove_item_e.grid(row=1,column=1)

        #delete price
        self.delete_item = Label(aif,text="Delete Price", font=("arial",12,"bold"),bg="white")
        self.delete_item.grid(row=1,column=2,sticky=W,padx=5,pady=2)
        #delete price entry
        self.add_item_e = ttk.Entry(aif, font=("arial",10),width=24)
        self.add_item_e.grid(row=1,column=3)

        #add item to category botton
        btn_ad = ttk.Button(aif,text="Add Items",cursor="hand2",command=lambda: self.add)
        btn_ad.grid(row=2,column=1)

        #remove item from category botton
        btn_ad = ttk.Button(aif,text="Remove Items",cursor="hand2")
        btn_ad.grid(row=2,column=3)
        #=====================================#
        ...
        #exit
        self.btn_e = Button(bt,text="Exit", font=("arial",15,"bold"),command=self.quit,bg="orangered",fg="white",height=2,width=12,cursor="hand2")
    ...
    def welcome(self):
        ...
        #class
        self.textarea.insert(END,f"\t\t\t\t\t Class: {self.class_c.get()}")
    ...
    def AddItem(self):
        ...
        self.n=int(self.rat_com.get())
        self.m=int(self.qua_com.get())* self.n
        self.l.append(self.m)
        if self.san_com.get()=="" and self.dri_com.get()=="":
            pass
        else:
            self.textarea.insert(END,f"\n{self.san_com.get()}{self.dri_com.get()}\t\t{self.qua_com.get()}\t\t{self.rat_com.get()}\t\t{self.m}\n")
            self.total.set(str('Rs.%.2f'%(sum(self.l))))
            self.san_com.set("")
            self.dri_com.set("")
            self.rat_com.set("")
            #self.pri_com.set("")
            self.qua_com.delete(0,"end")
    ...
    def gen_bill(self):
            text=self.textarea.get(8.0,(8.0+float(len(self.l)+5)))
            self.welcome()
